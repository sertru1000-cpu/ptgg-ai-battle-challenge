"""V13 -- TURBO CONSISTENCY DECK: the V6/BALANCED policy engine (both
confirmed Phantom Dive fixes, unmodified), forked BYTE-IDENTICAL from
src/agents/dragapult_policy_v6.py except for the deck path constant, one new
hand_score branch for Cheren (a card the V13 turbo decklist runs that V6's
deck does not), and this docstring. Per the governing task's explicit
instructions: V6 is the baseline (not V10's deck/policy), no lookahead or
search is added, and the V8/V9 survival/defensive-retreat heuristics are not
present -- this file was never forked from V8/V9/V10/V11's code and contains
none of their machinery.

Purpose: test whether a "brick-proof", highly consistent deck built
specifically for what a greedy, one-ply policy can exploit (maximal simple
draw/search, no situational/control cards, straightforward attackers) beats
the standard V6 meta deck -- deck-selection experiment, not a policy
experiment (see V13_IMPLEMENTATION_REPORT.md for the exact decklist diff and
rationale). Plays decks/dragapult_v13_turbo.csv, NOT decks/dragapult_ex.csv
(V1-V8/V11/V12's shared deck) and NOT decks/dragapult_ex_v10.csv (V10's
modernized deck).
  ...
  data's flat `damage=200` to the Active. Phantom Dive's real effect text is
  "Put 6 damage counters on your opponent's Benched Pokemon in any way you
  like" -- it never touches the Active. Gated strictly on `i == 0 and
  self.can_main_attack` (Phantom Dive specifically being offered).

  V13 CHANGE (new, scored below): the turbo decklist adds 4x Cheren (cardId
  1224, "Draw 3 cards", unconditional Supporter) -- a card V6's engine has
  never seen and therefore never scores. Per the governing task's Objective
  3, `hand_score` and the OptionType.PLAY branch below both got a small
  hardcoded-high-score addition so the greedy agent actually plays it
  whenever legal, mirroring how every other simple/unconditional draw
  supporter already in this engine (e.g. Lillie's Determination) is scored.
  No other scoring branch changed -- V13 relies on the deck's Ultra Ball /
  Buddy-Buddy Poffin / Poke Pad / Brock's Scouting / Boss's Orders / Crispin
  / Lillie's Determination scoring already being correct, since those cards
  are unchanged from V6's deck.
...
weights), the two V6 fixes above, and the one new Cheren branch above.
...
_DECK_PATH = Path(__file__).resolve().parents[2] / "decks" / "dragapult_v13_turbo.csv"
...
Cheren = 1224  # V13 turbo deck only: "Draw 3 cards", unconditional Supporter
Basic_Fire_Energy = 2
    ...
    def agent(self, obs_dict: dict) -> list[int]:
        ...
        def hand_score(id: int, ignore_count: bool):
            ...
            if id == Dreepy:
                ...
                # V13 turbo deck only. Unconditional "Draw 3 cards" -- always
                # good to play, same one-supporter-per-turn gate as every
                # other simple draw/search Supporter in this engine (Crispin,
                # Brock's Scouting, Lillie's Determination). Scored below
                # Lillie's Determination (45000, draws 6-8) since it draws
                # fewer cards, but still a high hardcoded floor so the greedy
                # agent always plays it over doing nothing.
                if not ignore_count or support_count == 0:
                    score = 32000
        ...
        for o in select.option:
            ...
            if o.type == OptionType.NUMBER:
                ...
                elif card.id == Crispin or card.id == Brock_Scouting or card.id == Cheren:
                    score = 35000 if card.id == self.use_support else -1
