"""V15 -- "Holy Grail Baseline": V6's fixed attack engine + V4's DEFENSIVE
weight profile.

Strict fork of src/agents/dragapult_policy_v6.py (byte-identical engine
logic; the two Phantom Dive fixes below are unchanged). V6 reached the
correct attack logic but shipped with the BALANCED weight profile (a
regression from V4's higher-ELO DEFENSIVE profile, introduced when V6 was
forked). V15 does not change a single line of engine/scoring logic here --
the DEFENSIVE-vs-BALANCED choice is made entirely by which PolicyWeights
value src/agents/dragapult_agent_v15.py passes into `make_agent()`; this
module remains weight-agnostic exactly as V6 was. Diff against
dragapult_policy_v6.py to confirm zero delta below this docstring.

Original V6 docstring (unchanged content, describes the two fixes this file
still carries):

V6 -- V2/BALANCED engine with the two confirmed Phantom Dive bugs fixed.

Forked from src/agents/dragapult_policy_v2plus.py (V2's shared engine, still
used unmodified by V2/V3/V4/V5) specifically so this causal A/B experiment
can isolate the effect of the two defects documented in
PHANTOM_DIVE_ARCHITECTURE_AUDIT.md and confirmed not to be an index-alignment
issue by PHANTOM_DIVE_INDEX_ALIGNMENT_AUDIT.md. Every line is identical to
dragapult_policy_v2plus.py except the two changes marked "V6 FIX #1" and "V6
FIX #2" below. Diff against dragapult_policy_v2plus.py to see the exact
delta.

  FIX #1 (§3.2): the DAMAGE_COUNTER_ANY fallback's `elif hp == 10: score -=
  100000` was backwards -- a target at exactly 10 HP is the cheapest
  possible kill (one damage counter) and should be strongly preferred, not
  penalized below unreachable targets.

  FIX #2 (§3.3): `main_option_proc`'s i==0 (opponent's Active) iteration
  treated Phantom Dive as if it deals its card data's flat `damage=200` to
  the Active. Phantom Dive's real effect text is "Put 6 damage counters on
  your opponent's Benched Pokemon in any way you like" -- it never touches
  the Active. That false premise could both fabricate a KO'd-active prize
  count and skip the bench-counter combo search entirely (`max_score =
  50000` shortcut), leaving `self.plan_b.counter` empty for the entire
  attack. Gated strictly on `i == 0 and self.can_main_attack` (Phantom Dive
  specifically being offered) so it cannot affect any other attack or the
  bench_attacker/switch-only path that also calls this function.
...
weights) and the two V6 fixes above.
