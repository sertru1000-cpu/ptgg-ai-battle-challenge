"""Shared, weight-parameterized Dragapult ex battle engine (Prompt #5, Part 4:
"shared battle engine + version-specific decision policy").
...
documented in src/agents/policy_weights.py, each of which is an identity
no-op at NEUTRAL weights. tools/verify_v2_engine_equivalence.py mechanically
proves this by feeding the real V1 agent and a NEUTRAL-weighted instance of
this class the exact same sequence of real-game observations and asserting
identical returned actions at every decision.
