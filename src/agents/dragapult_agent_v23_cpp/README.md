# V18 = V17 + Block A1 (opponent-archetype determinization)

**This package is a fork of `dragapult_agent_v17_cpp`.** The single functional
change: the opponent's hidden zones (deck/hand/prizes) are no longer
determinized from OUR OWN deck (V17's mirror-match assumption) -- the
opponent's archetype is classified per decision from their visible board +
discard (`opponent_model.py`, canonical lists embedded in
`archetype_decks.py`, regenerate via `tools/gen_v18_archetype_decks.py`), and
the sampling pool = that archetype's real 60-card list minus everything of
theirs already visible **including their discard** (which V17 never
subtracted). The pool crosses the ABI as `oppHiddenPoolIds` (see `v17_abi.h`,
magic bumped to "V18A"); `mcts.hpp::sample_opponent_hidden` uses it verbatim,
falling back to V17's mirror logic only if the pool is empty. Everything
below this line is the original V17 README and still accurate for the rest of
the architecture.
