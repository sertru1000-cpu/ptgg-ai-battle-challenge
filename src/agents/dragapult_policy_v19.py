"""V19 -- V6 engine adapted to the LEADER Dragapult decklist (the exact 60
cards played identically by 11+ of the top-100 ladder's 19 Dragapult teams,
including rank #2 "flg" at rating 1217 -- extracted from their real replay,
see results/leader_diff/deck_diff.md and decks/dragapult_ex_v19.csv).

Deck delta vs V6's list (the "Munkidori package"):
  +2 Basic {D} Energy, +2 Munkidori, +2 Jamming Tower, +1 Judge, +1 Dawn,
  +1 Poke Pad (3->4)
  -2 Rare Candy, -2 Brock's Scouting, -2 TR Watchtower, -1 Latias ex,
  -1 Lucky Helmet, -1 Crispin (4->3)

Policy delta vs dragapult_policy_v6.py (every change marked "V19:"):
  - Munkidori: bench it once, attach a {D} Energy to it, then use
    Adrena-Brain every turn it can move damage counters (engine-verified
    skill text: "move up to 3 damage counters from 1 of your Pokemon to 1 of
    your opponent's Pokemon" -- pure profit with Phantom Dive's counters and
    chip damage taken on our attackers);
  - {D} Energy: attaches ONLY to Munkidori (Phantom Dive needs {R}{P}; a
    dark on an attacker is a wasted slot);
  - Judge (shuffle-draw-4 both sides): situational disruption supporter,
    scored up when the opponent's hand is large and ours is weak;
  - Dawn (fetch a Basic + a Stage 1 + a Stage 2 into hand): line assembler,
    scored high while the Dreepy/Drakloak/Dragapult line is incomplete --
    it single-handedly replaces the removed Rare Candy fast path;
  - Jamming Tower: same play pattern as the removed TR Watchtower (we no
    longer run any Tools ourselves, so it is pure upside against tool decks);
  - dead V6 branches for removed cards (Rare Candy / Latias / Lucky Helmet /
    Brock's / Watchtower) are left in place, inert -- their hand/field/deck
    counts are simply always 0 with this decklist. Rare Candy's evolution
    fast-path references stay 0-count and never fire.

Below this docstring the file is dragapult_policy_v6.py with only the
"V19:"-marked insertions -- diff against it to see the exact delta.
"""
...
_DECK_PATH = Path(__file__).resolve().parents[2] / "decks" / "dragapult_ex_v19.csv"  # V19: leader list
...
# V19: the Munkidori-package additions (see module docstring). The removed
# cards' constants above are kept -- their branches are inert at 0 copies.
Basic_Dark_Energy = 7
Munkidori = 112
Judge = 1213
Dawn = 1231
...
class DragapultPolicy:
    ...
    def agent(self, obs_dict: dict) -> list[int]:
        ...
        def attach_score(attach_id: int, pokemon: Pokemon, active: bool) -> int:
            ...
            if pokemon.id == Budew:
                return -1
            # V19: Munkidori wants exactly one {D} (Adrena-Brain's requirement)
            # and nothing else; a {D} on any other Pokemon is a wasted slot
            # (Phantom Dive costs {R}{P}).
            if pokemon.id == Munkidori:
                if attach_id == Basic_Dark_Energy and not any(c.id == Basic_Dark_Energy for c in pokemon.energyCards):
                    return 20300
                return -1
        ...
        def hand_score(id: int, ignore_count: bool):
            ...
            if id == Dreepy:
                ...
                # V19: bench one Munkidori; the second copy is redundancy only.
                score = 12000 if field_counts[Munkidori] == 0 else 5
            elif id == Judge:
                # V19: shuffle-draw-4 disruption -- worth more the fatter the
                # opponent's hand is; never outranks our own draw engine when
                # their hand is small.
                if not ignore_count or support_count == 0:
                    score = 6000 + 2500 * max(0, op_state.handCount - 4)
            elif id == Dawn:
                # V19: fetches a Basic + a Stage 1 + a Stage 2 -- assembles the
                # whole Dreepy/Drakloak/Dragapult line in one card; replaces
                # the removed Rare Candy fast path as the line accelerator.
                if not ignore_count or support_count == 0:
                    line_missing = (
                        (deck_counts[Dreepy] > 0 and hand_counts[Dreepy] + field_counts[Dreepy] == 0)
                        or (deck_counts[Drakloak] > 0 and hand_counts[Drakloak] + field_counts[Drakloak] == 0)
                        or (deck_counts[Dragapult_ex] > 0 and hand_counts[Dragapult_ex] + field_counts[Dragapult_ex] == 0)
                    )
                    score = 46000 if line_missing else 8000
            elif id == Jamming_Tower:
                # V19: same pattern as the (removed) TR Watchtower branch -- we
                # run no Tools in this list, so it is pure upside vs tool decks.
                if stadium_id != 0 and stadium_id != Jamming_Tower:
                    score = 4000
            elif id == Basic_Dark_Energy:
                # V19: only ever for Munkidori (attach_score enforces that);
                # same -5000 "prefer non-energy plays first" idiom as {R}/{P}.
                max_score = -10000
        ...
        for o in select.option:
            ...
            if o.type == OptionType.NUMBER:
                ...
                if card is not None:
                    ...
                    if context == SelectContext.SWITCH or context == SelectContext.TO_ACTIVE or context == SelectContext.SETUP_ACTIVE_POKEMON:
                        ...
                        if card.id == Dragapult_ex:
                            score += 200
                    elif effect_card_id == Munkidori and isinstance(card, Pokemon):
                        # V19: Adrena-Brain's source/target selects (context id
                        # not pinned down -- effect-gated so this cannot leak
                        # into any other flow). Source: our MOST damaged
                        # Pokemon (maximum counters to ship). Target: their
                        # most killable Pokemon, same shape as the
                        # DAMAGE_COUNTER scoring above.
                        if o.playerIndex == my_index:
                            score = card.maxHp - card.hp
                        else:
                            score = 100000 - 10 * card.hp + pokemon_score(card, False, weights)
                ...
                if card.id == Dreepy:
                    ...
                    # V19: bench the first copy freely; never a second.
                    score = 50500 if field_counts[Munkidori] == 0 else -1
                elif card.id == Judge:
                    score = 14000 if card.id == self.use_support else -1
                elif card.id == Dawn:
                    # V19: line assembler -- when it IS the chosen supporter,
                    # prioritize it like Boss (it directly enables the attack plan).
                    score = 34000 if card.id == self.use_support else -1
                ...
                if card is not None and card.id == Munkidori:
                    # V19: Adrena-Brain -- checked BEFORE the no_draw guard
                    # (it moves damage counters, it never draws). Use it any
                    # turn at least one of our Pokemon actually carries damage
                    # to ship across; otherwise it has nothing to move.
                    damaged = any(
                        p is not None and p.hp < p.maxHp
                        for p in list(my_state.active) + list(my_state.bench)
                    )
                    score = 45000 if damaged else -1
                elif no_draw:
                    score = -1
