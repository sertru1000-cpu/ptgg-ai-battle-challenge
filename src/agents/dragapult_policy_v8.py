"""V8 -- V7 engine (all three Phantom Dive fixes, unmodified) plus exactly
ONE new strategic addition: a context-aware 2-Prize Defensive Retreat /
Survival heuristic. V7 itself is NOT modified by this file -- see
src/agents/dragapult_policy_v7.py, still the live experiment baseline.

Forked from src/agents/dragapult_policy_v7.py specifically so this causal A/B
experiment can isolate the effect of the one new heuristic. Every line is
identical to dragapult_policy_v7.py except the change marked "V8 SURVIVAL
RETREAT" below (the `_wants_survival_retreat`/`_bench_pokemon_is_ready`
methods, ~15 lines of new bookkeeping state in `__init__`/`agent()`, and one
new line wiring the hook into the existing `do_switch` computation). Diff
against dragapult_policy_v7.py to see the exact delta. Full design rationale,
audit of the pre-existing retreat/switch logic, and validation results are in
V8_RETREAT_HEURISTIC_AUDIT.md.

Hypothesis under test: "When our Active 2-Prize Pokemon is facing a reliable
(currently VISIBLE, not guessed) lethal threat and we have a genuinely
playable attacker on the Bench, V2/V6/V7 sometimes incorrectly choose ATTACK
instead of RETREAT, unnecessarily donating 2 Prize cards." This is a
consequence of `_wants_defensive_retreat` (V4/V5's own existing hook, left
byte-identical below) unconditionally returning False whenever
`self.can_attack` is True ("never give up a legal attack this turn to retreat
defensively") -- so that hook can *never* fire in exactly the situation this
hypothesis describes. `_wants_survival_retreat` is new, separately gated code
that specifically covers that gap; it does not alter `_wants_defensive_retreat`
or the situations that hook already handles.
...
class DragapultPolicy:
    ...
    def __init__(self, weights: PolicyWeights, adaptive: bool = False):
        ...
        # V8-only: anti-thrash memory for `_wants_survival_retreat` (STEP 4
        # guardrail). `_survival_retreat_pending` marks that the hook fired
        # on a still-unresolved decision; once a *different* Active is next
        # observed (the retreat has resolved), its serial/hp are snapshotted
        # so a later call can detect "this is the exact same Pokemon, at the
        # exact same HP, facing the exact same opponent Pokemon we already
        # retreated to save" and decline to re-fire on stale information.
        ...
        self.survival_retreat_log: list[dict] = []  # STEP 8 instrumentation, local-only
    ...
    # -- V8 SURVIVAL RETREAT: new context-aware 2-Prize survival heuristic --
    #
    # The one strategic addition this file makes over V7. Fires ONLY when
    # every condition below holds; any single failure leaves `do_switch`
    # exactly as V7 would have computed it. Does not touch, call, or change
    # the behavior of `_wants_defensive_retreat` above (V4/V5's own hook,
    # left byte-identical) -- this is new, additive code for the specific
    # gap that hook's own `if self.can_attack: return False` line leaves
    # open. See V8_RETREAT_HEURISTIC_AUDIT.md for the full design rationale,
    # the condition-by-condition trace back to the phase prompt's STEP 2,
    # and validation results.

    @staticmethod
    def _bench_pokemon_is_ready(pokemon: Pokemon) -> bool:
        """STEP 2.E: is this Bench Pokemon a genuinely playable attacker if
        it becomes Active right now -- i.e. does it already have enough
        attached energy (visible, ours) to legally fire at least one of its
        own real (damage > 0) attacks. Does not consider HP/bulk: ranking
        which ready candidate is actually the *best* one to retreat to is
        left entirely to the existing, unmodified SWITCH/TO_ACTIVE scoring
        block a few hundred lines below (STEP 1 audit finding: that ranking
        logic is untouched by V8).
        """
    ...
    def _wants_survival_retreat(
        ...
        """STEP 2: trigger a 2-Prize survival retreat even though we could
        still legally attack this turn (the gap `_wants_defensive_retreat`
        leaves open). Every lettered condition below is a hard AND-gate
        mapped 1:1 to the phase prompt's STEP 2.A-F; the first one that
        fails returns False immediately, and the whole hook stays fully
        inert (never changes `do_switch`) unless every one of them passes.
        """
        if my_active is None or op_active is None:
            return False

        # D: a legal retreat action must actually be on offer right now --
        # the engine itself is the source of truth for retreat legality
        # (asleep/paralyzed/just-played/no-legal-cost etc. are all already
        # reflected in whether OptionType.RETREAT appears this decision;
        # STEP 1's audit found no reason to duplicate that check here).
        ...
        # A: scoped to 2-Prize (ex, non-Mega-ex) Actives only -- STEP 5
        # explicitly excludes ordinary 1-Prize Pokemon (and, by the same
        # literal reading, 3-Prize Mega ex) from this heuristic.
        if not my_card.ex or my_card.megaEx:
            return False

        # B/C: the opponent's Active must have a realistically available
        # attack -- energy already visibly attached, per `cg.api`'s
        # documented sanitization (opponent Active/Bench are fully visible
        # once played; only their hand/deck are hidden) -- whose damage
        # meets or exceeds our Active's CURRENT hp. No hidden-hand guessing:
        # only `op_active.energies` (already-attached, visible) is read.
        op_energy_count = len(op_active.energies)
        lethal_confirmed = False
        for aid in op_card.attacks:
            if aid == 154:
                # Phantom Dive's card-data `damage=200` is fictional for the
                # Active -- it places up to six 10-damage counters on the
                # BENCH only and never touches the Active (source-verified,
                # this same file's own V6 FIX #2 / V7 FIX #3). Trusting the
                # raw card-data value here would resurrect that exact bug for
                # lethal-detection instead of KO-planning -- found by real
                # self-play (mirror Dragapult ex matchup) during Step 9
                # validation, not assumed. `_wants_defensive_retreat` above
                # has this same latent inaccuracy; left untouched per the
                # surgical scope (see V8_RETREAT_HEURISTIC_AUDIT.md, Known
                # Limitations).
                continue
            atk = _attack_table.get(aid)
            if atk is None or atk.damage <= 0:
            ...
            if op_energy_count >= len(atk.energies) and atk.damage >= my_active.hp:
                ...
                break
        if not lethal_confirmed:
            return False

        # Do not give up a winning trade: if OUR Active can itself KO the
        # opponent's Active this turn with its own already-attached energy,
        # attacking is still the better play than fleeing (not a literal
        # STEP 2 letter, but required so this hook cannot make V8 strictly
        # worse than V7 in a case the hypothesis was never about -- see the
        # audit report's "beyond-the-literal-spec" section).
        my_energy_count = len(my_active.energies)
        for aid in my_card.attacks:
            if aid == 154:
                continue  # Phantom Dive never damages the opposing Active either -- same fact as above
            atk = _attack_table.get(aid)
            if atk is None or atk.damage <= 0:
                continue
        ...
        # F: energy economy is explicitly inspected (and logged -- STEP 8)
        # rather than silently ignored. It is not an independent hard gate
        # beyond E above: once a genuinely ready Bench replacement exists,
        # losing the retreating 2-Prize Pokemon's attached energy is always
        # cheaper than losing the Pokemon itself plus its 2 Prizes to a
        # confirmed lethal hit. See V8_RETREAT_HEURISTIC_AUDIT.md, "Step 6"
        # section, for the worked-example reasoning behind this call.
        retreat_cost = my_card.retreatCost
        energy_lost = min(retreat_cost, my_energy_count)

        # If a survival-retreat decision is already pending resolution (we
        # decided to fire this same hook on an earlier MAIN call this same
        # turn, before the engine has actually executed the retreat and
        # produced a new Active), keep affirming it without re-running the
        # STEP 4 check or logging a duplicate entry -- there is no new
        # information yet.
        if self._survival_retreat_pending:
            return True

        # STEP 4 guardrail: decline to re-fire for the exact same Pokemon,
        # at the exact same HP, facing the exact same opponent Pokemon, that
        # we already retreated in to save -- nothing has actually changed,
        # so re-litigating would just thrash. A genuinely new threat (HP
        # moved, or the opponent's Active is a different Pokemon) is never
        ...
        self.survival_retreat_log.append(
            {
                ...
                "op_active_energy_count": op_energy_count,
    ...
    def agent(self, obs_dict: dict) -> list[int]:
        ...
        if state.turn == 0:
            ...
            # V8-only: a new game means the anti-thrash memory below refers
            # to a previous, unrelated battle -- never let it suppress a
            # legitimate retreat in this new game.
        ...
        # V8-only: STEP 4 anti-thrash bookkeeping, runs every call (matches
        # the existing unconditional-per-call pattern already used above for
        # my_active/op_active/do_switch). Detects that a survival retreat
        # decided on an earlier call has now resolved into a different
        # Active, and snapshots its identity/hp/opponent so
        # `_wants_survival_retreat` can recognize "nothing has actually
        # changed since we already retreated to save this exact Pokemon
        # from this exact opponent Pokemon" the next time it's evaluated.
        ...
        if not do_switch and context == SelectContext.MAIN:
            # V8 SURVIVAL RETREAT: the one new strategic hook, gated to the
            # actual MAIN action-selection decision (the only place
            # OptionType.RETREAT/ATTACK compete against each other), mirroring
            # how `main_option_proc` above is itself already scoped to
