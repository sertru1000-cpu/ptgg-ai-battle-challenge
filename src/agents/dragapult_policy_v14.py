"""V14 -- DYNAMIC GAME-PHASE POLICY. Forked from src/agents/dragapult_policy_v6.py
...
unchanged from V6 except at the explicit phase-aware hook points documented
below and in V14_IMPLEMENTATION_REPORT.md. Deliberately contains NO lookahead
...
hook is carried over unmodified, exactly as in V6 and V12, and is not touched
by any V14-specific logic.

WHAT'S NEW (Objectives 2-3 of the V14 task):

1. `detect_phase()`: a lightweight, per-decision read of both players' own
   remaining prize counts (`len(state.players[i].prize)`, real engine data --
   each side's own prize pile size is always visible, never hidden opponent
   info) plus the turn counter, classifying the current decision into EARLY,
   MID, or LATE game. LATE (either side <=2 prizes -- lethal urgency) is
   checked first and always wins regardless of turn count; a `turn <= 3`
   fallback (per the governing task's explicit "or turn counter <= 3"
   condition) forces EARLY next; otherwise MID fires when either side is at
   3 or 4 prizes, and EARLY is the remaining default (mechanically only
   reachable when both sides are at 5 or 6, since prize counts are integers
   in [0, 6] and neither of the first two branches matched).
2. `PhaseWeights`: wraps V6's existing `PolicyWeights` (kept fixed at
   BALANCED, V6's own profile -- V14 does not change V6's base tuning) with
   four new multipliers/bonuses, each identity (1.0 multiplier / 0.0 bonus)
   at the MID profile, so `PHASE_MID` reproduces V6's exact scoring formulas
   ("Use standard V6 default weights" is the task's own MID-phase
   instruction). Recomputed and applied at the top of every `agent()` call,
   never persisted stale across turns, same pattern V12 already validated
   for its own (orthogonal) matchup-detection hook.

One deliberate deviation from a strict line-for-line V6 copy, same one V12
already made and for the same reason: V6 carries a V5-only in-battle
opponent-aggression model (`_observe_opponent` / `_compute_adaptive_weights`,
gated behind `adaptive=False` and therefore dead code whenever `adaptive` is
off). That mechanism is orthogonal to this task (game phase, not opponent
aggression) and, wired through unchanged, would create a latent type
mismatch the moment `self.weights` stopped being a `PhaseWeights` (every
downstream call site here expects `.base.<field>`). Removed rather than left
in place with a landmine; V14 has no `adaptive` mode.
"""
...
# Confirms V14 plays the EXACT SAME deck as V6 -- see decks/dragapult_ex.csv,
...
# ---------------------------------------------------------------------------
# Objective 2: game-phase detection
# ---------------------------------------------------------------------------
...
def detect_phase(my_prizes: int, op_prizes: int, turn: int) -> str:
    """Classify the current decision by both players' own remaining prize
    counts (each side's `len(state.players[i].prize)` -- real, always-visible
    engine data, never hidden info) plus the turn counter.

    Priority order, per the governing task's own condition list:
      1. LATE ("either player has 1 or 2 prizes remaining") is checked
         first and wins unconditionally -- lethal urgency matters more than
         turn count or which prize bucket the other side is in.
      2. `turn <= 3` (the task's explicit "or turn counter <= 3" alternative
         EARLY-qualifying condition) forces EARLY next. In practice this
         branch is very rarely load-bearing on its own: no attack is legal
         before turn 2 at the earliest (needs an energy attachment first),
         so both sides' prize counts are still 5 or 6 through turn 3 in every
         real game observed during this task's local testing -- the
         prize-count rule below would already return EARLY in that case. It
         is kept as an explicit, separate branch anyway so the phase read
         is correct by construction even in a hypothetical early-KO edge
         case, per the task's literal wording, not just by empirical luck.
      3. MID ("3 or 4 prizes remaining") fires when either side is down to
         3 or 4 prizes.
      4. EARLY is the remaining default -- mechanically only reachable here
         when both sides are still at 5 or 6 (prize counts are integers in
         [0, 6]; neither branch 1 nor branch 3 matched), i.e. exactly
         "both players have 5 or 6 prizes remaining".
    """
...
# ---------------------------------------------------------------------------
# Objective 3: dynamic weight modulation by phase
# ---------------------------------------------------------------------------


@dataclass(frozen=True)
class PhaseWeights:
    """Wraps V6's `PolicyWeights` (kept at BALANCED -- V14 does not retune
    V6's own base profile) with phase-specific adjustments. Every new field
    is identity (0.0 bonus / 1.0 multiplier) at `PHASE_MID_WEIGHTS`, so MID
    reproduces V6's exact scores -- "Use standard V6 default weights" is the
    task's own instruction for the MID phase. See
    V14_IMPLEMENTATION_REPORT.md for the exact call sites each field touches
    and why.
    """
...
# EARLY: "Boost scores for card-draw supporters and basic Pokemon placement
# (Buddy-Buddy Poffin / Nest Ball equivalents). Lower the relative penalty
# for passive development turns (doing setup instead of weak early
# attacks)." `draw_bonus`/`basic_bonus` implement the first sentence
# directly (see hand_score()'s Crispin/Brock's Scouting/Lillie's
# Determination and Dreepy/Buddy-Buddy Poffin branches below). The second
# sentence -- "lower the relative penalty for passive turns" -- has no
# separate knob: V6's own ATTACK-option score is already just the raw
# `attackId` (a small integer, e.g. 154 for Phantom Dive), already far below
# any real setup action's score (thousands to tens of thousands), so
# attacking already never crowds out setup within a turn's decision loop.
# Boosting the setup-action scores below directly widens that existing gap
# in EARLY specifically, which both is the literal ask and avoids inventing
# an artificial "attack penalty" that doesn't otherwise exist in V6's
# scoring structure.
...
# LATE: "Absolute lethal priority, taking the final prizes at any cost,
# discarding preservation rules." `prize_aggression_multiplier` implements
# "aggressively boost attack scores and damage multipliers" (scales
# pokemon_score()'s prize term, which feeds both the attack-combo planner in
# main_option_proc() and the DAMAGE_COUNTER targeting score).
# `switch_risk_relief` implements "remove penalties for retreating or
# sacrificing a secondary Pokemon if it clears the path for a game-winning
# prize" (shrinks the fixed penalty V6 already applies to switching in a
# risky support Pokemon like Fezandipiti ex / Meowth ex).
...
def phase_weights_for(phase: str) -> PhaseWeights:
    if phase == PHASE_EARLY:
        return PHASE_EARLY_WEIGHTS
    if phase == PHASE_LATE:
        return PHASE_LATE_WEIGHTS
    return PHASE_MID_WEIGHTS
...
def pokemon_score(pokemon: Pokemon, is_attack_damage: bool, weights: PhaseWeights) -> int:
    """Heuristically evaluates the tactical worth of targeting a specific Pokemon on the opponent's field.

    Identical to V6 except the prize-count term is scaled by BOTH
    `weights.base.prize_value_multiplier` (unchanged from V6, always 1.0 at
    BALANCED) and `weights.prize_aggression_multiplier` (V14's own LATE-phase
    lethal-priority knob, identity 1.0 outside LATE).
    """
    data = _card_table[pokemon.id]
    score = prize_count(pokemon, is_attack_damage) * 1000 * weights.base.prize_value_multiplier * weights.prize_aggression_multiplier
...
class DragapultPolicy:
    ...
    isolation rationale as V6/V12 (never share an instance's `agent` method
    ...
    def __init__(self, weights: PolicyWeights):
        ...
        # PhaseWeights immediately so every downstream call site has a
        # single consistent type from turn 0 onward.
        self.base_weights = weights
        self.weights: PhaseWeights = PhaseWeights(base=weights)
        self.detected_phase: str = PHASE_EARLY
    ...
    # heavy hit. Carried over completely unmodified -- V14 does not add,
    ...
    def agent(self, obs_dict: dict) -> list[int]:
        ...
        # Objective 2 + 3: detect the current game phase from both players'
        # own remaining prize counts (+ turn counter) and shift weights
        # accordingly, before any scoring below runs. Recomputed every
        # decision (not cached) since prize counts change as the game
        # progresses.
        self.detected_phase = detect_phase(len(my_state.prize), len(op_state.prize), state.turn)
        self.weights = phase_weights_for(self.detected_phase)
        weights = self.weights
        ...
        def hand_score(id: int, ignore_count: bool):
            score = 0
            if id == Dreepy:
                score = 1000 if main_pokemon_count >= 3 else 18000 + weights.basic_bonus
            elif id == Drakloak:
                ...
                if count == 0:
                    ...
                    if state.turn <= 2 and field_counts[Budew] == 0 and deck_counts[Budew] >= 1:
                        count += 1
                    if count >= 2:
                        score = 35000 + weights.basic_bonus
                ...
                if not ignore_count or support_count == 0:
                    ...
                    if not self.can_main_attack and not self.bench_attacker and field_counts[Dragapult_ex] >= 1:
                        score = 55000 + weights.draw_bonus
                    else:
                        score = 25000 + weights.draw_bonus
            elif id == Brock_Scouting:
                if not ignore_count or support_count == 0:
                    if state.turn == 2 and field_counts[Budew] + field_counts[Latias_ex] == 0:
                        score = 50000 + weights.draw_bonus
                    else:
                        score = 30000 + weights.draw_bonus
            elif id == Lillie_Determination:
                if not ignore_count or support_count == 0:
                    score = 45000 + weights.draw_bonus
        ...
        for o in select.option:
            ...
            if o.type == OptionType.NUMBER:
                    ...
                    if context == SelectContext.SWITCH or context == SelectContext.TO_ACTIVE or context == SelectContext.SETUP_ACTIVE_POKEMON:
                        if o.playerIndex == my_index:
                            if card.id == Dreepy:
                                ...
                                score -= 1000 / (weights.base.switch_risk_tolerance * weights.switch_risk_relief)
                            elif card.id == Meowth_ex:
                                score -= 2000 / (weights.base.switch_risk_tolerance * weights.switch_risk_relief)
                        else:
                            if self.plan_a.attack == o.index + 1:
                                score += 100000
                        score += energy_count * 1000
                        score += hp * (1 + weights.base.preservation_bias)
                ...
                if card.id == Dreepy:
                    # EARLY hook: "basic Pokemon placement" -- boosts playing
                    # Dreepy from hand to the bench. Identity 0.0 outside
                    # EARLY.
                    score = 51000 + weights.basic_bonus
                elif card.id == Fezandipiti_ex:
                    ...
                    # EARLY hook: card-draw Supporter boost. Identity 0.0
                    # outside EARLY.
                    score = 14000 + weights.draw_bonus if card.id == self.use_support else -1
                elif card.id == Team_Rocket_Watchtower:
                    score = 80000 if (stadium_id > 0 or state.turn == 1) else -1
                elif no_draw:
                    score = -1
                elif card.id == Buddy_Buddy_Poffin:
                    # EARLY hook: "basic Pokemon placement" (bench building).
                    # Identity 0.0 outside EARLY.
                    score = 46000 + weights.basic_bonus if deck_counts[Dreepy] > 0 else -1
                    ...
                    # EARLY hook: card-draw/search Supporter boost. Identity
                    # 0.0 outside EARLY.
                    score = 35000 + weights.draw_bonus if card.id == self.use_support else -1
                ...
                score = o.attackId
                if self.plan_a.attack > 0:
                    # LATE hook: "aggressively boost attack scores ... take
                    # the final prizes at any cost." When main_option_proc
                    # has already identified a productive Phantom Dive
                    # target/counter-combo this turn (plan_a.attack > 0),
                    # scale that signal into the ATTACK option's own score so
                    # it can outcompete marginal, non-essential setup actions
                    # (a spare Crushing Hammer, a low-value energy attach
                    # beyond what this attack needs, an optional retreat) that
                    # would otherwise be picked first purely because their
                    # flat scores exceed ATTACK's tiny baseline `o.attackId`.
                    # Identity 0.0 bonus outside LATE (prize_aggression_
                    # multiplier == 1.0 at EARLY/MID).
                    score += (weights.prize_aggression_multiplier - 1.0) * 20000
...
def make_agent(weights: PolicyWeights, always_first: bool = True):
    ...
    (BALANCED) the policy wraps for every phase profile -- V14 does not add
    an `adaptive` mode (see module docstring). Exposes `.policy` (the live
    instance, useful for tests/inspection, including `.policy.detected_phase`)
    and `.DECK`.
    """
    ...
    fn.__name__ = f"dragapult_v14_{weights.name}"
    return fn
