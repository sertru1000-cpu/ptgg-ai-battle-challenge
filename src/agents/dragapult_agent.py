"""Baseline agent #3: Dragapult ex.

Ported from the official sample notebook
`sample_notebooks/a-sample-rule-based-agent-dragapult-ex-deck.ipynb`
("Advanced Level"). Behavior-preserving port: scoring/planning logic
unchanged, refactored only to use src.agents.common's shared helpers and load
its deck from an explicit path (decks/dragapult_ex.csv, hand-constructed from
the notebook's named-constant decklist, counts independently summed to
exactly 60 and confirmed legal via battle_start: errorType=0).

Uses select_top (not select_top_simple): this notebook's own tail can decline
down to select.minCount when only negative-scoring bench candidates remain
(SelectContext.TO_BENCH / SETUP_BENCH_POKEMON only) -- see
src/agents/common.py's docstrings.

Known limitation carried over from the notebook, not fixed here: this module
uses cross-call `global` state (card_counts, turn logs, attack plans), so two
players sharing the same imported module instance (a mirror match) would
corrupt each other's tracking. Not used in a mirror-match pairing this phase.
"""
...
# Cross-call state (mirrors the notebook's module-level globals exactly)
can_switch = False
can_attack = False
...
def agent(obs_dict: dict) -> list[int]:
    ...
    global pre_turn_log, current_turn_log, bench_attacker, use_support
