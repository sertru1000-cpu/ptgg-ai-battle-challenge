  ...
  try {
    ...
    if (eligible_for_search) {
      ...
      if (static_cast<int>(candidates.size()) >= 2) {
        // V24: full deep-tree PUCT search (tree_search.hpp) replaces the
        // one-level Mcts entirely. mcts.hpp remains in the build as the
        // provider of shared config history but is no longer on this path.
        v17::DeepTreeConfig cfg;
        cfg.time_budget_ms = time_budget_ms;
        v17::DeepTree tree(*g_engine, g_cards, g_deck, cfg);
        std::string sbi(search_begin_input, static_cast<size_t>(search_begin_input_len));
        int visits = 0;
        bool begin_ok = false;
        bool hit_terminal = false;  // deep tree reports depth instead; kept for ABI stability
        int begin_fail_count = 0;   // unused on this path
        int begin_last_error = 0;
        int max_depth = 0;
        int chosen = tree.search(root_obs, candidates, sbi, *g_live_scorer, &visits, &begin_ok, &max_depth);
        hit_terminal = max_depth > 0;  // repurposed: "search actually descended"
        if (out_begin_fail_count) *out_begin_fail_count = begin_fail_count;
        if (out_begin_last_error) *out_begin_last_error = max_depth;  // repurposed diagnostic: max tree depth
...
// V20 debug/parity export: vectorize the given root observation with the C++
// vectorizer and run the embedded B1 forest on it. Local parity testing only
// (Python compares this against src/ml/vectorizer.py + xgboost on the same
// live observation) -- never on the decision path. `out_features` (nullable,
// capacity 87) also returns the raw feature vector so a probability mismatch
// can be localized to the exact feature.
V17_API int v20_debug_pwin(const V17RootObservationC* obs_c, double* out_pwin, float* out_features) {
  if (!g_initialized || !obs_c || !out_pwin) return 1;
  if (obs_c->abi_magic != V17_ABI_MAGIC) return 2;
  try {
    v17::Observation obs = v17::from_ctypes(*obs_c);
    float features[v20::kVecFeatureCount];
    v20::vectorize_observation(obs, g_cards, features);
    if (out_features) {
      for (int i = 0; i < v20::kVecFeatureCount; ++i) out_features[i] = features[i];
    }
    *out_pwin = v20::pwin_predict(features);
    return 0;
  } catch (...) {
    return 3;
  }
