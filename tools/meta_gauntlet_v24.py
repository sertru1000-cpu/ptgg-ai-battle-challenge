"""v24 meta-deck gauntlet -- the Block-A1 measurement run (V17_ROADMAP.md:
...
Usage:
    python tools/meta_gauntlet_v24.py --games-per-deck 20 --games-vs-v12 30
"""
...
from src.agents.dragapult_agent_v24_cpp import native_bridge  # noqa: E402

DECKS_DIR = REPO_ROOT / "decks"
OUT_DIR = REPO_ROOT / "results" / "v24_meta_gauntlet"
...
def _reset_match_budgets() -> None:
    import src.agents.final_candidate_agent_v24 as v24_mod

    v24_mod._timeout_shielded_agent.stats["cumulative_elapsed_seconds"] = 0.0


def run_series(series_label: str, opp_name: str, opp_fn, opp_deck, n_games: int,
               v24_name: str, v24_fn, v24_deck, games_f) -> dict:
    wins = losses = draws = aborted = 0
    for i in range(n_games):
        v24_slot0 = i % 2 == 0  # strict alternation
        native_bridge.CLASSIFICATION_COUNTS.clear()
        result = _play_one_game_keep_search_input(
            run_id=f"v24_meta_gauntlet_{series_label}",
            game_index=i,
            agent_a_name=v24_name,
            agent_a_fn=v24_fn,
            deck_a=v24_deck,
        ...
        if result.aborted:
            aborted += 1
        elif result.winner_agent == v24_name:
            wins += 1
        ...
        games_f.flush()
        print(f"[{series_label}] [{i + 1}/{n_games}] v24_slot0={v24_slot0} "
              f"winner={result.winner_agent} aborted={result.aborted} classif={classif}", flush=True)
    return {"series": series_label, "games": n_games, "v24_wins": wins, "v24_losses": losses,
...
def main() -> None:
    ...
    v24_name, v24_fn, v24_deck = load_agent("src.agents.final_candidate_agent_v24", name_override="v24_arch_det")
    ...
    for deck_label, deck_path, v6_control_wins, v17_wins in META_DECKS:
        ...
        row = run_series(deck_path.stem, f"baseline__{deck_path.stem}", baseline_fn, baseline_deck,
                         args.games_per_deck, v24_name, v24_fn, v24_deck, games_f)
        row["deck_label"] = deck_label
        row["v6_control_wins_of_20"] = v6_control_wins
        row["v17_wins_of_20"] = v17_wins
        summary.append(row)
        print(f"=== {deck_label}: v24 {row['v24_wins']}W-{row['v24_losses']}L "
    ...
    if args.games_vs_v12 > 0:
        ...
        row = run_series("vs_v12", v12_name, v12_fn, v12_deck, args.games_vs_v12,
                         v24_name, v24_fn, v24_deck, games_f)
        row["deck_label"] = "V12 head-to-head"
        summary.append(row)
        print(f"=== V12 head-to-head: v24 {row['v24_wins']}W-{row['v24_losses']}L ===", flush=True)
