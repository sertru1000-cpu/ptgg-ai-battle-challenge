"""V18 meta-deck gauntlet -- the Block-A1 measurement run (V17_ROADMAP.md:
...
Usage:
    python tools/meta_gauntlet_v18.py --games-per-deck 20 --games-vs-v12 30
"""
...
from src.agents.dragapult_agent_v18_cpp import native_bridge  # noqa: E402

DECKS_DIR = REPO_ROOT / "decks"
OUT_DIR = REPO_ROOT / "results" / "v18_meta_gauntlet"
...
def _reset_match_budgets() -> None:
    import src.agents.final_candidate_agent_v18 as v18_mod

    v18_mod._timeout_shielded_agent.stats["cumulative_elapsed_seconds"] = 0.0


def run_series(series_label: str, opp_name: str, opp_fn, opp_deck, n_games: int,
               v18_name: str, v18_fn, v18_deck, games_f) -> dict:
    wins = losses = draws = aborted = 0
    for i in range(n_games):
        v18_slot0 = i % 2 == 0  # strict alternation
        native_bridge.CLASSIFICATION_COUNTS.clear()
        result = _play_one_game_keep_search_input(
            run_id=f"v18_meta_gauntlet_{series_label}",
            game_index=i,
            agent_a_name=v18_name,
            agent_a_fn=v18_fn,
            deck_a=v18_deck,
        ...
        if result.aborted:
            aborted += 1
        elif result.winner_agent == v18_name:
            wins += 1
        ...
        games_f.flush()
        print(f"[{series_label}] [{i + 1}/{n_games}] v18_slot0={v18_slot0} "
              f"winner={result.winner_agent} aborted={result.aborted} classif={classif}", flush=True)
    return {"series": series_label, "games": n_games, "v18_wins": wins, "v18_losses": losses,
...
def main() -> None:
    ...
    v18_name, v18_fn, v18_deck = load_agent("src.agents.final_candidate_agent_v18", name_override="v18_arch_det")
    ...
    for deck_label, deck_path, v6_control_wins, v17_wins in META_DECKS:
        ...
        row = run_series(deck_path.stem, f"baseline__{deck_path.stem}", baseline_fn, baseline_deck,
                         args.games_per_deck, v18_name, v18_fn, v18_deck, games_f)
        row["deck_label"] = deck_label
        row["v6_control_wins_of_20"] = v6_control_wins
        row["v17_wins_of_20"] = v17_wins
        summary.append(row)
        print(f"=== {deck_label}: V18 {row['v18_wins']}W-{row['v18_losses']}L "
    ...
    if args.games_vs_v12 > 0:
        ...
        row = run_series("vs_v12", v12_name, v12_fn, v12_deck, args.games_vs_v12,
                         v18_name, v18_fn, v18_deck, games_f)
        row["deck_label"] = "V12 head-to-head"
        summary.append(row)
        print(f"=== V12 head-to-head: V18 {row['v18_wins']}W-{row['v18_losses']}L ===", flush=True)
