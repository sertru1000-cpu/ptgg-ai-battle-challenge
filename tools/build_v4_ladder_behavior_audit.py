"""V4 real-ladder behavioral audit -- parse V4's real-ladder replays through the IDENTICAL
shared pipeline already used for Luca and V2 (src/meta_analysis/ladder_behavior_audit.py),
so V2 vs V4 is an honest same-methodology comparison, not two different analyses.

V4 Defensive's real games come from submission 55471848 (a 2026-08-13 resubmission of the
byte-identical challenger_v4_20260812T053037Z.tar.gz build originally submitted as 55449825,
which never got matched into real games -- see tools/pull_v4_ladder_data.py docstring for the
verification). 17 real EPISODE_TYPE_PUBLIC games (+1 validation, excluded).

Writes results/ladder_behavior_audit/v4_{games,decisions,missed_knockouts}.csv -- raw,
per-decision, never discarded after aggregation, matching the V2/Luca convention exactly.
Does NOT touch the existing v2_*/luca_*.csv outputs.
"""
...
TARGETS = [
    {
        "label": "v4",
        "submission_id": 55471848,
        "replay_dir": os.path.join(ROOT, "data", "v4_ladder_audit", "replays"),
        "episodes_file": os.path.join(ROOT, "data", "v4_ladder_audit", "v4_episodes_raw.json"),
        "opponent_ratings_file": os.path.join(ROOT, "data", "v4_ladder_audit", "opponent_ratings_raw.json"),
    },
    {
        # re-run with the added KO-conversion fields (attacks_thrown/kos_landed/ko_rate_per_attack)
        # so V2 and V4 are directly comparable per-game, not just in aggregate. Same source
        # replays already pulled for the V2-vs-Luca audit -- no new Kaggle calls, deterministic
        # re-parse of unchanged data (same pattern as session 11's frozen-pipeline re-runs).
        "label": "v2",
...
def run_target(target):
    ...
    for e in eps_meta:
        ...
        # KO conversion: of all ATTACK decisions made, how many resulted in the opponent's
        # active Pokemon HP hitting 0 (a knockout) by the very next observation.
        attacks_thrown = [d for d in decs if d["chosen_attack_ids"]]
        kos_landed = sum(1 for d in attacks_thrown if (d["opp_after"].get("active_hp") == 0) or (d["opp_after"].get("active_id") is None and d["opp_before"].get("active_id") is not None))
        ...
        row = {
            ...
            "attacks_thrown": len(attacks_thrown),
            "kos_landed": kos_landed,
            "ko_rate_per_attack": (kos_landed / len(attacks_thrown)) if attacks_thrown else None,
