"""Phantom Dive forensic audit -- V7 and V8 real Kaggle ladder games.

Byte-for-byte the same methodology as tools/build_phantom_dive_forensic_v6.py
(itself byte-for-byte the same as the original V2 script), retargeted at V7's
and V8's real ladder data so all four (V2/V6/V7/V8) stay directly comparable
on identical logic (immunity rules, allocation-optimality search, thresholds).

Reads data/{v7,v8}_ladder_audit/{label}_episodes_raw.json + replays/.
Writes results/phantom_dive_forensic_{v7,v8}/{events,missed_ko_examples}.csv
and phantom_dive_summary.json.
Read-only against Kaggle and against V7/V8 -- no agent/weight/deck changes.
"""
...
TARGETS = {
    "v7": {"submission_id": 55478172, "replay_dir": os.path.join(ROOT, "data", "v7_ladder_audit", "replays"),
           "episodes_file": os.path.join(ROOT, "data", "v7_ladder_audit", "v7_episodes_raw.json"),
           "out_dir": os.path.join(ROOT, "results", "phantom_dive_forensic_v7")},
    "v8": {"submission_id": 55482268, "replay_dir": os.path.join(ROOT, "data", "v8_ladder_audit", "replays"),
           "episodes_file": os.path.join(ROOT, "data", "v8_ladder_audit", "v8_episodes_raw.json"),
           "out_dir": os.path.join(ROOT, "results", "phantom_dive_forensic_v8")},
}
...
def run(label, cfg):
    ...
    epmeta = json.load(open(cfg["episodes_file"], encoding="utf-8"))
    all_events = []
    for e in epmeta:
    ...
    n_naive_ko_opportunity = sum(1 for e in with_targets if any(t["hp"] <= 60 for t in e["pre_targets"]))
    ...
    with open(os.path.join(cfg["out_dir"], "events.csv"), "w", newline="", encoding="utf-8") as f:
    ...
    for e in sorted(missed, key=lambda e: -(e["best_prize"] - e["actual_prize"])):
        ex_rows.append({
            ...
            "actual_allocation_by_slot": e["actual_counts_by_slot"],
            "actual_kos": [t["card_name"] for t in e["pre_targets"] if t["slot"] in e["actual_kos_slots"]],
            "actual_prize": e["actual_prize"],
    ...
    with open(os.path.join(cfg["out_dir"], "missed_ko_examples.csv"), "w", newline="", encoding="utf-8") as f:
        if ex_rows:
    ...
    with open(os.path.join(cfg["out_dir"], "phantom_dive_summary.json"), "w", encoding="utf-8") as f:
